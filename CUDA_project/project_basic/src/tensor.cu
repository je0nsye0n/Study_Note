...
Tensor::Tensor(const vector<size_t> &shape_) {
  ...
  cudaMalloc(&gbuf,sizeof(float) * num_elem());
}
...
Tensor::Tensor(const vector<size_t> &shape_, float *buf_) {
  ...
  cudaMalloc(&gbuf,sizeof(float) * num_elem());
  cudaMemcpy(gbuf,buf,sizeof(float)*num_elem(),cudaMemcpyHostToDevice);
}

void Tensor::toCPU(){
  cudaMemcpy(buf,gbuf,sizeof(float)*num_elem(),cudaMemcpyDeviceToHost);
